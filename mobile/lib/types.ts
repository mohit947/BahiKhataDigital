...
export interface Product {
  id: string; name: string; unit: string; rate: number;
  gst_rate: number; category?: string; is_active: boolean;
}

export interface BillItem {
  id: string; product_name: string; unit: string;
  quantity: number; rate: number; discount_percent: number;
  discount_amount: number; gst_percent: number; gst_amount: number; total: number;
}
...
export interface Bill {
  ...
  customer?: Customer; payment_logs: PaymentLog[];
}
