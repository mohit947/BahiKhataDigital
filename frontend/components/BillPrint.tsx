...
const BillPrint = forwardRef<HTMLDivElement, Props>(
  ({ bill, org, upiLink, customQrImage, orientation = "portrait" }, ref) => {
    const isLandscape = orientation === "landscape";
    const width = isLandscape ? "297mm" : "210mm";
    const minHeight = isLandscape ? "210mm" : "297mm";
    const padding = isLandscape ? "8mm 12mm" : "12mm 15mm";
    ...
    const orgUpiId = org?.upi_id || "";

    const showQr = !!(customQrImage || (upiLink && orgUpiId));

    return (
      <div
        ref={ref}
        className="bg-white text-gray-900 font-sans"
        style={{ width, minHeight, padding, boxSizing: "border-box", fontSize: "11px" }}
      >
        {/* Header */}
        <div className="flex justify-between items-start border-b-2 border-gray-900 pb-4 mb-4">
          <div>
            <h1 style={{ fontSize: "22px", fontWeight: 800 }}>{orgName}</h1>
            {orgAddress && (
              <p className="text-gray-600 mt-0.5" style={{ fontSize: "10px" }}>{orgAddress}</p>
            )}
            {(orgPhone || orgEmail) && (
        ...
        <div className="grid grid-cols-2 gap-4 mb-4">
          ...
          <div className="border border-gray-300 rounded-lg p-3">
            ...
            <div className="space-y-0.5">
              ...
              <p>Status: <span className="font-bold">{bill.payment_status.toUpperCase()}</span></p>
              {Number(bill.amount_paid) > 0 && (
                <p>Paid: <span className="font-medium">{formatCurrency(bill.amount_paid)}</span></p>
              )}
              {bill.payment_status !== "paid" && (
                <p>Balance: <span className="font-bold">
        ...
        <table className="w-full border-collapse mb-4" style={{ fontSize: "10px" }}>
          <thead>
            <tr style={{ backgroundColor: "#111", color: "white" }}>
              <th className="text-left py-2 px-2" style={{ width: "28px" }}>#</th>
              ...
              <th className="text-right py-2 px-2" style={{ width: "75px" }}>Amount (₹)</th>
            </tr>
          </thead>
          <tbody>
            {bill.items.map((item, idx) => (
              <tr key={item.id} style={{ backgroundColor: idx % 2 === 0 ? "#f5f5f5" : "white" }}>
        ...
        {/* Bottom Section: GST Summary + QR + Totals */}
        <div className="grid grid-cols-3 gap-4 mt-2">
          {/* GST Summary */}
          <div className="col-span-1">
            {Object.keys(gstSummary).length > 0 && (
              <div className="border border-gray-300 rounded-lg overflow-hidden" style={{ fontSize: "9px" }}>
            ...
            {bill.notes && (
              <div className="border border-gray-300 rounded-lg p-3 mt-2" style={{ fontSize: "9px" }}>
          ...
          {/* QR Code */}
          <div className="col-span-1 flex flex-col items-center justify-center border border-gray-300 rounded-lg p-3">
            {customQrImage ? (
              <>
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={customQrImage} alt="Payment QR" style={{ width: 90, height: 90, objectFit: "contain" }} />
                <p className="text-center mt-1 text-gray-500" style={{ fontSize: "8px" }}>Scan to Pay</p>
              </>
            ) : showQr ? (
              <>
                <QRCode value={upiLink!} size={90} fgColor="#000000" bgColor="#ffffff" />
                <p className="text-center mt-1 text-gray-500" style={{ fontSize: "8px" }}>
                  Scan to Pay via UPI
                </p>
                <p className="text-center font-medium" style={{ fontSize: "9px" }}>
                  {orgUpiId}
                </p>
              </>
            ) : (
          ...
          {/* Totals */}
          <div className="col-span-1">
            <div className="border border-gray-300 rounded-lg overflow-hidden" style={{ fontSize: "10px" }}>
            ...
            {/* Amount in Words */}
            <div className="mt-2 border border-gray-300 rounded-lg p-2" style={{ fontSize: "9px" }}>
        ...
        <div className="text-center mt-4 pt-2 border-t border-gray-100">
          <p className="text-gray-400" style={{ fontSize: "8px" }}>
            This is a computer-generated invoice. Thank you for your business with {orgName}!
          </p>
        </div>
      </div>
    );
